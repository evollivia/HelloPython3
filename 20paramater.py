...
# 즉, 매개변수는 입력도니 데이터 값을 함수에 제공하는 역할을 담당
...
# 파이썬 에서는 *args, **kwargs라는 구문 사용
# *args -> 튜플, **kwargs -> dict 형태로 전달됨
...
# 언패킹
# 시퀀스 자료형이나 dict등의 값을 개별변수에 할당하는 과정
...
uid, pid = member.values()
print(uid, pid)
uid, pid = member.keys()
print(uid, pid)
uid, pid = member.items()  # py3.8 이상만 가능
print(uid, pid)
...
print_kwargs(**mydict)  # dict의 값들을 개별적으로 출력


# 매개변수 기본값
# 함수 정의시 매개변수의 기본값을 지정하면
# 함수 호출히 매개변수에 인수가 할당되지 않을 경우,
# 기본값이 대신 할당됨 - 함수를 좀더 유연하게 사용 가능
...
def squr(x):
    # 출력까지 함수에서 구현하기 때문에
    # 플랫폼이 변하는 경우 함수를 재작성 해야하는 번거로움 발생
    print(f'{x} => {x ** 2}')
...
# 즉, return문 이후의 코드는 실행되지 않음 - 함수 조기종료 가능
