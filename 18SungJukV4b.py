...
# 이름, 국어, 영어, 수학을 입력받으면
# 총점, 평균, 학점을 계산하고 출력함
# CRUD기능을 지원하는 성적처리 프로그램으로 재작성
# 즉, 성적 입력, 조회, 상세조회, 수정, 삭제 기능을 구현
# 각 기능은 메뉴식으로 구현 - 기능별 메뉴 선택시 해당 명령 수행
# 학생의 성적데이터는 개별 변수가 아닌 리스트 안에 dict 형태로 저장
import sys
...
while True:
    ...
    if menu == '1':
        ...
        print('성적 데이터 추가')
        cnt = len(sjs)
        sj['name'] = input(f'{cnt}학생 이름 : ')
        sj['kor'] = int(input(f'{cnt}학생 국어 성적 : '))
        sj['eng'] = int(input(f'{cnt}학생 영어 성적 : '))
        sj['mat'] = int(input(f'{cnt}학생 수학 성적 : '))
